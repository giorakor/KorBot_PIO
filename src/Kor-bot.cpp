...
orientation get_orientation()
{
  ...
  if (abs(gyro_Pitch_vel - prev_gyro_Pitch_vel) > MAX_JUMP_IN_PITCH)
    gyro_Pitch_vel = prev_gyro_Pitch_vel;
  if (abs(gyro_Yaw_vel - prev_gyro_Yaw_vel) > 200)
    gyro_Yaw_vel = prev_gyro_Yaw_vel;
  ...
  {
    if (acc_fail_counter > 10)
      alpha_beta(gyro_Pitch_angle, accAngleY, ALPHA_GYRO_ACCEL);
    if (acc_fail_counter < 30)
      acc_fail_counter++;
  }
...
void filter_deadband_stick(float &filtered_stick_val, float raw_stick_val, float max_value)
{
  ...
  if (raw_stick_val > JOYSTICK_DEAD_BAND)
    stick_val_deadbanded = raw_stick_val - JOYSTICK_DEAD_BAND;
  if (raw_stick_val < -JOYSTICK_DEAD_BAND)
    stick_val_deadbanded = raw_stick_val + JOYSTICK_DEAD_BAND;
...
void filter_switch_transients(bool SW_status, bool &SWX, uint8_t switch_num)
{
  ...
  if (SW_status != prev_SW_status[switch_num])
    counter[switch_num] = 0;
...
void calc_PID_errors()
{
  ...
  if (prev_error * error < 0 || !motion_enable)
    integral_res = 0;
...
void calc_wanted_velocity()
{
  ...
  if (!motion_enable)
    wanted_velocity_from_alg_m_s = 0;
}
...
void read_user_commands()
{
  ...
  if (filtered_stick_velocity > prev_wanted_velocity && prev_wanted_acceleration > MAX_JERK_USR_MSSS * deltaT)
  {
    ...
    if (parabolic_extreme_velocity >= filtered_stick_velocity)
      max_acceleration = prev_wanted_acceleration - MAX_JERK_USR_MSSS * deltaT;
  }
  ...
  if (filtered_stick_velocity < prev_wanted_velocity && prev_wanted_acceleration < -MAX_JERK_USR_MSSS * deltaT)
  {
    parabolic_extreme_velocity = prev_wanted_velocity - pow(prev_wanted_acceleration, 2) / MAX_JERK_USR_MSSS / 2;
    if (parabolic_extreme_velocity <= filtered_stick_velocity)
      min_acceleration = prev_wanted_acceleration + MAX_JERK_USR_MSSS * deltaT;
  }
  ...
  if (SW1_pressed)
    toggle(motion_enable);
  if (!RemoteXY.button_1 && prev_button_1)
    toggle(motion_enable);
  prev_button_1 = RemoteXY.button_1;
  if (motion_enable != prev_motion_enable)
    reset_before_moving();
}
...
void setup()
{
  ...
  if (SEND_TELEMETRY)
    Serial.begin(115200); // Communication via USB to PC
...
void loop()
{
  ...
  if (prev_GUI_selector == 5)
    calibrate_gyro_online();
  first_run = false;
  toggle(other_cycle);
}